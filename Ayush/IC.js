 ...
 *   - NORMAL  orientation: pin 1 in row E (notch up)
 *   - FLIPPED orientation: pin 1 in row F (notch down)
 */
...
export class IC {
  ...
  static dipFootprint(startRow, startColumn, orientation = IC.ORIENTATION.NORMAL, pinCount = 14) {
    ...
    const pin1Row = orientation === IC.ORIENTATION.NORMAL ? IC.ROW.E : IC.ROW.F;
  ...
  static validate() {
    ...
    for (const part of parts) {
      const pins = this.footprintFor(part, 'E', 10, 'NORMAL');
      const half = pins.length / 2;
      if (pins.length !== this.pinCountFor(part)) allStraddle = false;
      if (!pins.slice(0, half).every((p) => p.row === 'E')) allStraddle = false;
      if (!pins.slice(half).every((p) => p.row === 'F')) allStraddle = false;
    ...
    const p16 = this.footprintFor('74151', 'E', 10, 'NORMAL');
    add('74151 uses a DIP-16 footprint (16 pins)', p16.length === 16, `${p16.length} pins`);

    const flipped = this.footprintFor('7408', 'F', 20, 'FLIPPED');
    add(
      'FLIPPED orientation anchored in row F',
      flipped[0].row === 'F' && flipped[0].pin === 1,
      `pin 1 @ ${flipped[0].row}${flipped[0].column}`
    );

    add(
      'DIP-16 fits within 63 columns',
      this.footprintFor('74151', 'E', 56, 'NORMAL').every((p) => p.column <= 63)
    );
    add(
      'Out-of-range start column exceeds boundary',
      this.footprintFor('74151', 'E', 57, 'NORMAL').some((p) => p.column > 63)
    );
