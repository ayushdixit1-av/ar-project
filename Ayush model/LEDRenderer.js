...
export class LEDRenderer {
  static get LIT_INTENSITY() {
    return 2.2;
  }

  static get UNLIT_INTENSITY() {
    return 0.05;
  }

  /**
   * Turn a built (non-ghost) LED group on or off, driven by the circuit
   * simulator's verdict for that LED. Safe to call every frame.
   */
  static setLit(group, lit) {
    const glow = this._find(group, 'led_glow');
    if (!glow || !glow.material) return;
    const target = lit ? this.LIT_INTENSITY : this.UNLIT_INTENSITY;
    if (glow.material.emissiveIntensity !== target) {
      glow.material.emissiveIntensity = target;
    }
  ...
  static build(anodePos, cathodePos, color, options = {}) {
    ...
    // Glow die inside the body. Cloned (not shared) for real LEDs so each
    // instance can be lit/dimmed independently by the circuit simulator.
    const glowMat = ghost ? this.ghostMaterialFor(color) : this.glowMaterialFor(color).clone();
    if (!ghost) glowMat.emissiveIntensity = this.UNLIT_INTENSITY;
