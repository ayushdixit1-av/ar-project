...
const circuitChecks = CircuitSimulator.validate();
printReport(
  { pass: circuitChecks.every((c) => c.ok), checks: circuitChecks },
  'CIRCUIT SIMULATOR SELF-REVIEW REPORT'
);
...
if (viewer) {
  const statusEl = document.getElementById('status');
  const icSetup = setupICPlacement(viewer, holes, field);
  icTool = icSetup.tool;
  ...
  // Wire up live circuit simulation: every wire/IC/LED/power action funnels
  // through showStatus (see above), which calls runSimulation() below.
  runSimulation = () => {
    const result = CircuitSimulator.evaluate({
      holes: allHoles,
      wires: wireSetup.manager.all(),
      ics: icSetup.manager.ics,
      leds: ledSetup.manager.all(),
      powerSupply: powerSetup.manager,
    });
    for (const led of ledSetup.manager.all()) {
      const group = ledSetup.manager.mesh(led.id);
      if (group) LEDRenderer.setLit(group, result.ledStates.get(led.id) === true);
    }
  };
  runSimulation();
