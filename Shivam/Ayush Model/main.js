...
import { LEDBarGenerator } from './LEDBarGenerator.js';
import { InputBarGenerator } from './InputBarGenerator.js';
...
const autoWiresByIC = new Map();

function setupICPlacement(viewer, holes, field, getWireTool, allHoles) {
  ...
  const tool = new ICPlacementTool({
    ...
    onICPlaced: (ic) => {
      ...
      for (const w of wires) {
        try {
          const wire = wTool.manager.add(w);
          const startMesh = allHoles.find(h => h.id === w.start)?.position;
          const endMesh = allHoles.find(h => h.id === w.end)?.position;
          
          if (startMesh && endMesh) {
            // Map 2D position (x, y) to 3D scene (x, z)
            const startPos = new THREE.Vector3(startMesh.x, 0, startMesh.y);
            const endPos = new THREE.Vector3(endMesh.x, 0, endMesh.y);
            const { mesh } = WireRenderer.build(startPos, endPos, w.color);
            mesh.name = wire.id;
            mesh.userData.wireId = wire.id;
            scene.add(mesh);
            wTool.manager.attachMesh(wire.id, mesh);
            addedIds.push(wire.id);
          }
        } catch (e) {
          console.warn('Auto-wire skipped:', e.message);
        }
      }
      if (addedIds.length > 0) {
        autoWiresByIC.set(ic.id, addedIds);
      }
    },
    onICRemoved: (ic) => {
      const wTool = getWireTool();
      if (!wTool) return;
      const wireIds = autoWiresByIC.get(ic.id);
      if (wireIds) {
        for (const id of wireIds) {
          if (wTool.manager.get(id)) {
            wTool.manager.remove(id);
          }
        }
        autoWiresByIC.delete(ic.id);
      }
    }
  });
...
function setupPowerTool(viewer, field, allHoles, statusEl, terminals) {
  const { renderer, camera, scene } = viewer;
...
function setupInteractiveSwitches(viewer, inputBarVisual) {
  ...
  renderer.domElement.addEventListener('pointerdown', (event) => {
    ...
    const intersects = raycaster.intersectObject(inputBarVisual, true);

    if (intersects.length > 0) {
      let object = intersects[0].object;
      if (object.userData && object.userData.parentSwitch) {
        const switchGroup = object.userData.parentSwitch;
        if (switchGroup.userData && switchGroup.userData.toggle) {
...
const ledBarHoles = LEDBarGenerator.generateHoles();
const ledBarVisual = LEDBarGenerator.buildVisuals(ledBarHoles);

const inputBarHoles = InputBarGenerator.generateHoles();
const inputBarVisual = InputBarGenerator.buildVisuals(inputBarHoles);

const allHoles = [...holes, ...power.holes, ...ledBarHoles, ...inputBarHoles];
const field = buildSphereField(allHoles);

const powerSupplyBuilt = PowerSupplyRenderer.buildSupply(null);

const boardContainer = new THREE.Group();
boardContainer.name = 'boardContainer';
boardContainer.add(field);
boardContainer.add(body);
boardContainer.add(ledBarVisual);
boardContainer.add(inputBarVisual);
boardContainer.add(powerSupplyBuilt.group);
...
if (viewer) {
  const statusEl = document.getElementById('status');
  icTool = setupICPlacement(viewer, holes, field, () => wireTool, allHoles);
  ...
  const powerSetup = setupPowerTool(viewer, field, allHoles, statusEl, powerSupplyBuilt.terminals);
  ...
  setupInteractiveSwitches(viewer, inputBarVisual);
}
