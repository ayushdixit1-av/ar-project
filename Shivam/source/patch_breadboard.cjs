...
const newTex = `function createBreadboardTopCanvasTexture(): THREE.CanvasTexture {
  ...
  if (ctx) {
    ...
    const drawTerminalStrip = (y: number, blockIdx: number) => {
      ...
      slotCenters.forEach((sc, idx) => {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(\`— [ IC SOCKET \${idx + 1 + blockIdx * 4} ] —\`, sc, y + 130);
      });
    ...
    drawTerminalStrip(126, 0);
    drawPowerStrip(432, false);
    drawTerminalStrip(526, 1);
    drawPowerStrip(832, false);
    drawTerminalStrip(926, 2);
    drawPowerStrip(1232, false);
  }
...
const regexBody = /const body = new THREE.Mesh\(new THREE.BoxGeometry\(11\.6, 0\.42, 2\.6\), breadboardMat\);/;
const newBody = `const body = new THREE.Mesh(new THREE.BoxGeometry(11.6, 0.42, 7.2), breadboardMat);`;
code = code.replace(regexBody, newBody);

const regexTopPlate = /const topPlate = new THREE.Mesh\(new THREE.PlaneGeometry\(11\.56, 2\.56\), topPlateMat\);/;
const newTopPlate = `const topPlate = new THREE.Mesh(new THREE.PlaneGeometry(11.56, 7.16), topPlateMat);`;
code = code.replace(regexTopPlate, newTopPlate);

const regexTrough = /const trough = new THREE.Mesh\([\s\S]*?new THREE\.BoxGeometry\(11\.2, 0\.12, 0\.24\),[\s\S]*?\);[\s\S]*?trough\.position\.set\(0, 0\.18, 0\);[\s\S]*?group\.add\(trough\);/;
const newTrough = `[0, 1, 2].forEach(block => {
    const trough = new THREE.Mesh(
      new THREE.BoxGeometry(11.2, 0.12, 0.24),
      new THREE.MeshStandardMaterial({ color: 0xcfc8b8, roughness: 0.7 })
    );
    // 126 is y-start. Trough center is y+130. 
    // center of canvas is 1350/2 = 675.
    // 130 - 675 = -545 canvas units.
    // In 3D space: 7.2 total depth / 1350 = 0.00533 per unit.
    // offset = (y_center - 675) * (7.2 / 1350)
    const blockCenterY = [126 + 130, 526 + 130, 926 + 130][block];
    trough.position.set(0, 0.18, (blockCenterY - 675) * (7.2 / 1350));
    group.add(trough);
  });`;
